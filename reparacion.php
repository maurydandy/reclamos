...
function Header()
{
	...
	//$this->Cell(100,10,'PERSONAL',0,0,'L');
...
class DB{
	...
	$strConsulta = "SELECT * from trabajo where id =  '$paciente'";
...
//id, cuit, expe, empresa, acta 
    ...
    $pdf->SetFont('Arial','B',10);
 	...
 	$pdf->Cell(0, 6,'Sr. Jefe de:',0,1);
 	$pdf->Cell(0, 6,'Oficina Computos',0,1);
    $pdf->Cell(0, 6,'Cristian Mattei',0,1);
    //$pdf->Cell(102, 8,'Sr. Cristian Mattei',0,1)
 	...
 	$pdf->Cell(0,6,'Me dirijo a usted para informar que el dia '.$fila['fecha' ]   .'..'.  $fila ['persona']  . ' de la oficina de '.$fila['oficina'] ,0,1);
   	$pdf->Cell(0,6, $fila['detalle'] ,0,1);
   	//$pdf->Cell(0,6,'bajo el nombre comercial de  '.  $fila['nfantasia']   .'  CUIT N  '   .  $fila['cuit']    .'  establecido en el rubro de '  .  $fila['rubro']   ,0,1);
   	$pdf->Cell(0, 6,'',0,1);
   	$pdf->Cell(0, 6,'',0,1);
   	$pdf->Cell(0, 6,'',0,1);
   	$pdf->Cell(0, 6,'',0,1);
   	$pdf->Cell(100,10,'Saluda ATTE' ,0,1);
   	$pdf->Cell(0,6, $fila['personal'] ,0,1);
